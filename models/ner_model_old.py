...
class NERModel(BaseModel):
    ...
    def add_words_chars_elmo_highway_op(self):

        with tf.variable_scope("words_emb"):
            if self.config.embeddings is None:
                self.logger.info("WARNING: randomly initializing word vectors")
                _word_embeddings = tf.get_variable(
                        name="_word_embeddings",
        ...
        elmo_embeddings = tf.concat([elmo_embeddings,word_embeddings, cnns_h], axis=-1)
        self.elmo_and_char_embeddings = tf.nn.dropout(elmo_embeddings, self.dropout)
    ...
    def add_logits_op_elmo_rdbilstm(self):
        ...
        output_h = tf.math.add(input_h, output)  # residual connection
        ...
        with tf.variable_scope('attention-rd'):
            attn, alphas = attention(output_rd)
            output_rd = tf.expand_dims(attn, axis=1) * output_rd
    ...
    def build(self):
        if self.config.use_elmo:
            ...
            if self.config.use_elmo_and_words:
                self.add_words_chars_elmo_highway_op()
            else:
